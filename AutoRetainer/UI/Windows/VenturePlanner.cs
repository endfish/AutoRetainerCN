    ...
    public override void Draw()
    {
        ...
        if(SelectedRetainer != null && SelectedCharacter != null)
        {
            ...
            if(adata.LinkedVenturePlan == "")
            {
                ...
                if(ImGui.Checkbox("启用计划器", ref adata.EnablePlanner))
                {
                ...
                {
                    ImGuiEx.SetNextItemFullWidth();
                    if(ImGui.BeginCombo("##load", "加载已保存计划...", ImGuiComboFlags.HeightLarge))
                    {
                        int? toRem = null;
                ...
                if(adata.VenturePlan.List.Count > 0)
                {
                    ...
                    }, delegate
                    {
                        ...
                        ImGuiEx.Tooltip(overwrite ? "覆盖已有探险计划" : $"保存探险计划");
                    });
                }
                ...
                if(ImGui.BeginCombo("##addVenture", "添加探险...", ImGuiComboFlags.HeightLarge))
                {
                    ...
                    ImGui.SameLine();
                    ImGuiEx.SetNextItemFullWidth();
                    ImGuiEx.EnumCombo("##unavail", ref C.UnavailableVentureDisplay, Lang.UnavailableVentureDisplayNames);
                    if(ImGui.BeginChild("##ventureCh", new(ImGui.GetContentRegionAvail().X, ImGuiHelpers.MainViewport.Size.Y / 3)))
                    {
                        ...
                        if(ImGui.Button($"{Lang.CharDice}    自由探索", ImGuiHelpers.GetButtonSize("A") with { X = ImGui.GetContentRegionAvail().X }))
                        {
                ...
                if(adata.EnablePlanner && adata.VenturePlan.ListUnwrapped.Count > 0)
                {
                    ...
                    ImGui.SameLine();
                    ImGuiEx.Tooltip("取消此计划中剩余的探险，并从头开始");
            ...
            {
                ImGuiEx.TextWrapped($"此雇员的探险计划与另一个雇员的探险计划共享。");
            }
        }
